...
try:
    ...
    HAS_OPENCV = False
    print("OpenCV chưa được cài. Hãy chạy: pip install opencv-python-headless")
...
class ScannerScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Khởi tạo OpenCV detector
        self.detector = None
        if HAS_OPENCV:
            self.detector = cv2.QRCodeDetector()
            # Có thể dùng thêm để decode cả Data Matrix nếu cần
    ...
    def on_enter(self):
        ...
        if not HAS_OPENCV:
            self.status_label.text = "❌ OpenCV chưa được cài đặt!"
            return
    ...
    def _init_camera(self):
        ...
        try:
            ...
            try:
                ...
                self.status_label.text = f"Lỗi camera: {str(e2)[:40]}"
                print(f"Camera init error: {e2}")
    ...
    def _start_camera_play(self, dt):
        ...
        try:
            if self.camera:
                ...
                Clock.schedule_interval(self.scan_frame, 0.3)
        except Exception as e:
    ...
    def scan_frame(self, dt):
        """
        Quét frame từ camera bằng OpenCV QRCodeDetector
        Hỗ trợ: QR Code, Data Matrix (qua ZBar), Code 128, EAN-13, v.v...
        """
        ...
        try:
            ...
            # Lấy dữ liệu pixel từ texture Kivy
            ...
            # Chuyển texture Kivy (RGBA) sang numpy array cho OpenCV
            img = np.frombuffer(pixels_data, dtype=np.uint8).reshape(height, width, 4)
            
            # Chuyển RGBA -> BGR (OpenCV xử lý BGR)
            img_bgr = cv2.cvtColor(img, cv2.COLOR_RGBA2BGR)
            
            # Thử decode với nhiều góc xoay để bắt QR từ mọi hướng
            ...
            for angle in rotations:
                try:
                    ...
                    # ---- DÙNG OPENCV QRCODE DETECTOR ----
                    # QRCodeDetector nhạy hơn với QR code
                    data, bbox, _ = self.detector.detectAndDecode(rotated)
                    if data:
                        decoded_data = data
                        break
                    
                    # ---- NẾU KHÔNG TÌM THẤY QR, THỬ DÙNG BARCODE DETECTOR ----
                    # Có thể decode cả Code 128, EAN-13, Data Matrix, v.v...
                    try:
                        # Khởi tạo BarcodeDetector (có sẵn trong OpenCV 4.x)
                        barcode_detector = cv2.barcode.BarcodeDetector()
                        ret, decoded_info, decoded_type = barcode_detector.detectAndDecode(rotated)
                        if ret and decoded_info and len(decoded_info) > 0:
                            decoded_data = decoded_info[0]
                            break
                    except:
                        # Nếu BarcodeDetector không có sẵn, bỏ qua
                        pass
                    
                except Exception as e:
                    print(f"Rotation {angle} error: {e}")
                    continue
            
            # Nếu tìm thấy dữ liệu
            if decoded_data:
                # Làm sạch dữ liệu
                data = decoded_data.strip()
            ...
            self.status_label.text = "OpenCV chưa được cài!"
            self.is_scanning = False
            Clock.unschedule(self.scan_frame)
        except Exception as e:
            # Lỗi scan không ảnh hưởng đến luồng chính
    ...
    def open_scanner(self, *args):
        ...
        if not HAS_OPENCV:
            Popup(title="Thông báo", 
                  content=Label(text="OpenCV chưa được cài đặt!\nVui lòng cài:\npip install opencv-python-headless"),
                  size_hint=(.8,.5)).open()
            return
